...
def preprocess(df):
    ...
    X = df.drop(columns=["Position", "grid_position", "is_top10_start", "grid_vs_team_avg", "driver_avg_quali_last5", "team_avg_quali_last5"], errors="ignore")
...
def main():
    ...
    for tag, df in all_data.items():
        ...
        final_results[tag] = evaluate_models(X_train, X_test, y_train, y_test, X_full, y, MODEL_DIR, prefix=f"{tag}_top1_pre-quali")

    output_file = OUTPUT_DIR / "model_performance_top1_pre_quali.json"
