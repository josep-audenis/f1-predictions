...
MODEL_DIR = Path(__file__).resolve().parent.parent / ".." / "models" / "top1" / "quali"
...
def main():
    ...
    for tag, df in all_data.items():
        ...
        final_results[tag] = evaluate_models(X_train, X_test, y_train, y_test, X_full, y, MODEL_DIR, prefix=f"{tag}_top1_quali")

    with open(OUTPUT_DIR / "model_performance_top1_quali.json", "w") as f:
